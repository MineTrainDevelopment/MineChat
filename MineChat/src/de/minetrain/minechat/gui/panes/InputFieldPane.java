...
//			getMessageHistory().addSendedMessages(message.getMessageRaw());
			MessageManager.sendMessage(Main.getChannelManager().getActiveChannelActions(), inputField.getPlainText());
			inputField.clear();
		});
...
//        inputField.prefHeightProperty().bind(Bindings.createDoubleBinding(() -> text.getBoundsInLocal().getHeight(), text.boundsInLocalProperty()).add(20));
		...
		MineTextFlow infoBox = new MineTextFlow().setDefaultFontSize(18d)
...
//	        .appendString(" -  Version: "+Main.VERSION, HTMLColors.GRAY)
		...
		infoBox.clear().setAlignment(TextAlignment.LEFT)
//	        .appendString("Reply to thrad from: ", Color.AQUA)
//	        .appendString("Pferdchen", Color.LIGHTPINK)
				.appendString("Reply to ", Color.AQUA).appendString("@MineTrainLP", Color.GREENYELLOW)
				.appendString(" in thrad from: ", Color.AQUA).appendString("@Pferdchen", Color.LIGHTPINK)
				.appendString(":", Color.WHITE).appendLineSplit()
...
//        infoBox.prefColumnCountProperty().bind(infoBox.textProperty().length());
		...
		setCenter(inputField); // input field
